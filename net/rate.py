...
class SuddenCyclicScheduler():
    def __init__(self, base_lr=0.002, max_lr=0.01, step=2000,start_step=0,gamma=1):
        self.base_lr = base_lr
        self.max_lr = max_lr
        self.step = step
        self.gamma = gamma
    ...
    def get_rate(self, epoch=None, num_epoches=None):
        if self.trn_iterations<=self.start_step:
            self.trn_iterations+=1
            return self.max_lr
        clr_step=(self.trn_iterations-self.start_step)%self.step
        lr=(-self.max_lr+self.base_lr)/self.step*clr_step+self.max_lr
        self.trn_iterations += 1
...
def adjust_learning_rate(optimizer, lr):
    for index,param_group in enumerate(optimizer.param_groups):
        # print('param group in adjust lr',index)
...
if __name__ == '__main__':
    ...
    scheduler = SuddenCyclicScheduler(base_lr=0.002,max_lr=0.01,step=25,start_step=5)
    #scheduler = DecayScheduler(base_lr=0.1, decay=0.32, step=10)
    #scheduler = CyclicScheduler(base_lr=0.0001, max_lr=0.01, step=5., mode='triangular', gamma=1., scale_fn=None, scale_mode='cycle') ##exp_range ##triangular2


    lrs = np.zeros((num_epoches),np.float32)
    for epoch in range(num_epoches):

        lr = scheduler.get_rate(epoch*num_its)
        lrs[epoch] = lr
        if lr<0:
            num_epoches = epoch
            break
