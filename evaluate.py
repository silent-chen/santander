...
def do_kaggle_metric(predict,truth, threshold=0.5):
    ...
    threshold = np.array([0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.90])
    for t in threshold:
        p = iou>=t
...
def run_check_run_length_encode():
    ...
    df = pd.read_csv ( 'E:\\DHWorkStation\\Project\\tgs_pytorch\\data\\train.csv')
    ...
    for n in df.id.values:
        mask_file = 'E:\\DHWorkStation\\Project\\tgs_pytorch\\data/train/masks/' + n +'.png'
...
def run_check_run_length_decode():
    ...
    df = pd.read_csv ( 'E:\\DHWorkStation\\Project\\tgs_pytorch\\data/train.csv')
    #df = df.loc[df['id'].isin(name)]
    df = df.fillna('')
