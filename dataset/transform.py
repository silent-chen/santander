...
def do_center_pad(image, size1,size2):
    image = cv2.copyMakeBorder(image, size1, size2, size1, size2, cv2.BORDER_REFLECT_101)
    return image


def do_center_pad2(image, mask, size1,size2):
    image = do_center_pad(image, size1,size2)
    mask = do_center_pad(mask, size1,size2)
    return image, mask
